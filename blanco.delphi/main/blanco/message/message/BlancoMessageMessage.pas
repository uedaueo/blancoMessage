...
type
    ...
    TBlancoMessageMessage = class(TObject)
        ...
        const MBMSGI01: String = '[MBMSGI01] メッセージ定義ID[{0}]のパッケージ名が指定されていません。';

        // キー値: KEY_MBMSGI01
        const KEY_MBMSGI01: String = 'MBMSGI01';

        // 文字列[サポートしない出力プログラミング言語処理系[{0}]が指定されました。]
        const MBMSGI02: String = '[MBMSGI02] サポートしない出力プログラミング言語処理系[{0}]が指定されました。';

        // キー値: KEY_MBMSGI02
        const KEY_MBMSGI02: String = 'MBMSGI02';

        // 文字列[メッセージ定義ID[{0}]において、キー[{1}]が2回以上あらわれました。同じキーは2回以上指定することはできません。]
        const MBMSGI03: String = '[MBMSGI03] メッセージ定義ID[{0}]において、キー[{1}]が2回以上あらわれました。同じキーは2回以上指定することはできません。';
        ...
        const MBMSGI04: String = '[MBMSGI04] メッセージ定義ID[{0}]において、キー[{1}]のメッセージが不正です。メッセージ定義として判断した場合の置換文字列の個数[{2}]とリソースバンドルとして判断した場合の置換文字列の個数[{3}]とが異なります。置換文字列の番号は飛んでしまっていませんか？正しくフォーマット定義してください。';

        // キー値: KEY_MBMSGI04
        const KEY_MBMSGI04: String = 'MBMSGI04';

        // 文字列[メタディレクトリ[{0}]が存在しません。]
        const MBMSGA01: String = '[MBMSGA01] メタディレクトリ[{0}]が存在しません。';

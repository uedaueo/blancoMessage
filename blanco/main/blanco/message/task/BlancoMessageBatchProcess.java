...
/**
 * バッチ処理クラス [BlancoMessageBatchProcess]。
 *
 * <P>バッチ処理の呼び出し例。</P>
 * <code>
 * java -classpath (クラスパス) blanco.message.task.BlancoMessageBatchProcess -help
 * </code>
 */
public class BlancoMessageBatchProcess {
    ...
    public static final void main(final String[] args) {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoMessageBatchProcess: 入力パラメータ[" + arg + "]は無視されました。");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoMessageBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
    ...
    public final int execute(final BlancoMessageProcessInput input) throws IllegalArgumentException {
        try {
            ...
            System.out.println("BlancoMessageBatchProcess: 入力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoMessageBatchProcess: 入出力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoMessageBatchProcess: ランタイム例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            ex.printStackTrace();
            // 異常終了。
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoMessageBatchProcess: ランタイムエラーが発生しました。バッチ処理を中断します。:" + er.toString());
            er.printStackTrace();
            // 異常終了。
            return END_ERROR;
        }
    ...
    public static final void usage() {
        ...
        System.out.println("      説明[ターゲットとなるプログラミング言語処理系名。java, cs, js, vb,php,ruby,pythonが選択可能。]");
        System.out.println("      型[文字列]");
        System.out.println("      デフォルト値[java]");
        System.out.println("    -constants");
        System.out.println("      説明[文字列をクラスの定数として出力するかどうか。]");
        System.out.println("      型[真偽]");
        System.out.println("      デフォルト値[false]");
        System.out.println("    -isJavaJsf");
        System.out.println("      説明[Java JSF のメッセージを出力するかどうか。Java 言語のみ対応。]");
        System.out.println("      型[真偽]");
        System.out.println("      デフォルト値[false]");
    ...
    public void validateInput(final BlancoMessageProcessInput input) throws IllegalArgumentException {
        ...
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoMessageBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
        }
    }
}
